...
// La funzione decode_bits prende un array di char contenente 8 bit come parametro e decodifica i bit in un carattere utilizzando operatori bitwise. La funzione utilizza un ciclo while che itera 8 volte per decodificare ogni bit del carattere. All'interno del ciclo, viene verificato se il bit corrente è uguale a '1'. Se è uguale a '1', viene eseguito un OR bitwise tra la variabile c e il risultato dello shift a sinistra di 1 di i posizioni. Altrimenti, viene eseguito un OR bitwise tra la variabile c e 0. Alla fine del ciclo, viene restituito il valore della variabile c.
...
// La funzione sig_handler gestisce gli eventi di segnale inviati al processo. Quando viene ricevuto un segnale SIGUSR1 o SIGUSR2, questa funzione aggiunge un bit corrispondente alla variabile globale g_received_bits. Se la sequenza di bit raggiunge 8 caratteri, la sequenza viene decodificata in un carattere utilizzando la funzione decode_bits e stampata sull'output standard utilizzando la funzione write.
...
// La funzione main è la funzione principale per il processo server. La funzione inizializza un gestore di segnali per SIGUSR1 e SIGUSR2 e attende segnali in arrivo. Una volta ricevuto un segnale, viene chiamata la funzione gestore del segnale e il bit corrispondente viene aggiunto a un array globale di char. Una volta ricevuti 8 bit, vengono decodificati in un carattere e stampati sull'output standard utilizzando la funzione write.
